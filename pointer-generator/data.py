...
def example_generator(data_path, ner_path, single_pass):
...
def get_word_id_to_importance(ner_file, vocab):

     word_id_to_imp = dict()
     if (len(ner_file) > 0 and (path.exists(ner_file[0]))):
         with open(ner_file, "w") as ner_file[0]:
             lines = f.read().split('\n')
             for line in lines:
                if len(line) <= 0:
                     continue
                word = line.split()[0]
                imp = line.split()[1]
                word_id = vocab.word2id(vocab)
                word_id_to_imp[word_id] = imp
     return word_id_to_imp
