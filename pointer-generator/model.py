...
class SummarizationModel(object):
  ...
  def _reduce_states(self, fw_st, bw_st):
    ...
    with tf.variable_scope('reduce_final_st'):
      ...
      w_reduce_h = tf.get_variable('w_reduce_hsetup_training', [hidden_dim * 2, hidden_dim], dtype=tf.float32, initializer=self.trunc_norm_init)
  ...
  def _calc_final_dist(self, vocab_dists, attn_dists, word_id_to_imp):
    ...
    with tf.variable_scope('final_distribution'):
      ...
      # attn_dists = [(1-p_gen) * dist * (1 + self._w_m * imps.get(word_id, 0)) for (p_gen, dist, imps, word_id) in zip(self.p_gens, attn_dists, word_id_to_imp, "word_id")]
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      self._w_m = tf.get_variable("_w_m", [1])
      ...
      if FLAGS.pointer_gen:
        final_dists = self._calc_final_dist(vocab_dists, self.attn_dists, self.word_id_to_imp)
  ...
  def build_graph(self):
    ...
    # print("shoaib here", self._hps.mode)
